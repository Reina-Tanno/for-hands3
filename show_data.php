<?php
    ...
    try {
        ...
        //特定の年月のデータを抽出.
        $dbh = new PDO('mysql:host=localhost;dbname=Customers;charset=utf8;','root','akogi0304');
        $sql = "SELECT * FROM customers WHERE date_format(reserve_date,'%Y%m') = '201908'";
        ...
        foreach ($list as $row) {
            for($i = 1; $i < 16; $i++){
                if($row['reserve_date'] == '2019-08-0'.$i){ //20190701~20190709
                ...
                }else if($row['reserve_date'] == '2019-08-'.$i){//20190710~20190715
                    for($k = 9; $k < 24; $k++){
                        if($row['start_time'] == '0'.$k.':00:00'){
                            array_push($stack,$i);
                            array_push($stack_start,$k);
