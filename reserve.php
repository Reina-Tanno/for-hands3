    ...
    ?>
      	...
      	<body>
            <form name="yearmonth" method="POST" action="">
                <div id="yearmonth">
                <div id="yearr">
                    <h1><?php echo $ydate?>年</h1></div>
                <input type="hidden" id="yeara" name="yeara" value='+<?php echo $ydate?>+'>
                <!-- この画面の年はこのydateに格納する、あとで加工必要あり. -->
                <div id="monthh">
                    <h1><?php echo $mdate?>月</h1></div>
            ...
            <script type="text/javascript">
                ...
                thead.innerHTML = '<tr><th></th><th></th>' + th_item + '<th>備考</th>';
                ...
                for (var k = 0; k < 15; k++) {
                    ...
                    var kk = parseInt(k) + 1;
                    var tbody_item = '<div id="wrapper"><tr><th><p class="date">' + kk + '</p></th><td></td>' + td_item + '</div>';
                ...
                for (var i = 0; i < array_count; i++) { //for構文かいし. //9
                    ...
                    for (var l = 9; l < 24; l++) { //11
                        if (stack_start == l) {
                            document.getElementById(alpha[stack - 1] + '_' + l).innerHTML = '<div class="container"><canvas class="canvas" style="background-color:red;"></canvas><div class="title"><p>'+ stack_title + '</p></div></div>';  //_10のところをあとで治す.
                        }
                    }
                }
                ...
                $('.td').on('click', function () { //クリックしたidを取得. idをlocalstorageに格納.
                    ...
                    for (var i = 9; i < 23; i++) { //開始時間をstart_hに取得.
                        reg = new RegExp(i);//正規オブジェクトを使うと、正規表現のmatchにおいて、変数が使える. .match( $
                        ...
                        document.getElementById('reserve_click').innerHTML = '<form id="loginForm" name="loginForm" method="POST" action=""> '
                            ...
                            + '<input type="text" name="year" id="year" maxlength="5" value=' +<?php echo $ydate;?> +'>年'
                                + '<input type="text" name="month" id="month" maxlength="5" value=' +<?php echo date("m");?> +'>月' //月の部分は、このページに表示されているもの.
                                    ...
                                    + '<br>'
                                    + 'タイトル<input type="text" name="title" id="title" maxlength="20" value="">'
                                    + '<br>'
                                    ...
                                    + '<input type="submit" id="sub" name="sub" value="保存" onclick="fetchDeta()"></div></form>';
                    } else {
                        document.getElementById('reserve_click').innerHTML = '<form id="loginForm" name="loginForm" method="POST" action=""> '
                            + '<input type="hidden" name="year" value=' +<?php echo $ydate;?> +'>'
                                + '<input type="hidden" name="month" id="month" maxlength="5" value=' +<?php echo date("m");?> +'>'
                                    ...
                                    + '<input type="submit" id="del" name="del" value="削除する" onclick="fetchDeta()"></div></form>';
                    }
                });
